...
//we are counting to 6 ( *76ms ) so that we have ~500ms between switching the blinkers
...
// this timer decides which mode should be run ( online of offline )
// it basically increments the Dash_Activity counter if online mode is on
// if Dash_Activity reaches
...
void Update_Aux_State(aux_state *auxiliary, bool Toggle_State_Right,
					  bool Toggle_State_Left) {
	// the outputs are inverted

	// hazzards
	if (auxiliary->avarie == ON && Toggle_State_Left == Toggle_State_Right) {
		Sign_Right_500ms_Timer = Sign_Left_500ms_Timer; // syncing blinkers
	...
	HAL_GPIO_WritePin(GPIOA, FAN_Pin, (!auxiliary->fan));
					  ...
					  void Update_Buttons_State_Offline_Mode(aux_state *Offline_Mode) {
						  ...
						  // hazzards
						  if (HAL_GPIO_ReadPin(GPIOB, SIGN_LEFT_OFFLINE_MODE_Pin)
							  && HAL_GPIO_ReadPin(GPIOB, SIGN_RIGHT_OFFLINE_MODE_Pin)) {
							  Offline_Mode->avarie = ON;
							  } else
								  Offline_Mode->avarie = OFF;

						  // headlights
						  if (HAL_GPIO_ReadPin(GPIOB, FRONT_LIGHT_OFFLINE_MODE_Pin)
							  || HAL_GPIO_ReadPin(GPIOB, BACK_LIGHT_OFFLINE_MODE_Pin)) {
